...
function plaatsign_get_day_total($day, $month, $year) {
	...
	$sql  = 'select low_used, normal_used, solar_delivered, low_delivered, normal_delivered from energy_summary where date="'.$year.'-'.$month.'-'.$day.'"';	
	...
	if (isset($row->low_used)) {
		$low_used = $row->low_used;
	}	
	
	if (isset($row->normal_used)) {
	...
	return $low_used + $normal_used + $solar_delivered - $low_delivered - $normal_delivered;
}
...
function plaatsign_get_data($day, $month, $year) {
	...
	$sql  = 'select timestamp, power from energy1 where timestamp>="'.$timestamp1.'" and timestamp<="'.$timestamp2.'" order by timestamp';
	...
	while ($row = plaatsign_db_fetch_object($result)) {
		...
		if ( isset($row->power)) {
			$data[] = array(substr($row->timestamp,11,5), $row->power);
		}
	}		
	return $data;
}

function drawLegend($im, $text1, $cbar1, $font, $font_size)  {
	...
	imagefilledrectangle( $im, 450, $height-80 , 450+$size , $height-80+$size, $cbar1 );
	imagettftext($im, $font_size, 0, 470, $height-70, $black, $font, $text1);
}

function drawBars($im, $x, $y, $data, $color, $cbar1, $font, $font_size)  {
...
drawLabel($im, 0, 40, 'Energie Dagverbruik '.$day.'-'.$month.'-'.$year,  $fontArial, 30, $black);
