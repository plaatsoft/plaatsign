...
function firstDay() {
	...
	$sql = 'select date from energy_summary order by id asc limit 0,1';
...
function solarTotal() {
	...
	$sql = 'select sum(solar_delivered) as solar_delivered from energy_summary';

	$result = plaatsign_db_query($sql);
	$data = plaatsign_db_fetch_object($result);
	
	$total = 0;
	if (isset($data->solar_delivered)) {
		$total = $data->solar_delivered;
	}

	return $total;
}

function solarPeakPower() {

	$sql = 'select sum(value) as solar_peak_power from config where token="solar_peak_power"';

	$result = plaatsign_db_query($sql);
	$data = plaatsign_db_fetch_object($result);
	
	$total = 0;
	if (isset($data->solar_peak_power)) {
		$total = $data->solar_peak_power;
	}
...
function getSetting($token) {
	...
	$sql = 'select value from config where token="'.$token.'"';
...
drawLabel($im, 0, 40, 'Solar Systeem Samenvatting', $fontArial, 30, $black);
...
$y=120;
drawLabel($im, 170, $y, 'In bedrijfstelling', $fontArial, 24, $white);
$y+=$cellh;
drawLabel($im, 170, $y, 'Piekvermogen installatie', $fontArial, 24, $white);
$y+=$cellh;
drawLabel($im, 170, $y, 'Meet moment', $fontArial, 24, $white);
$y+=$cellh;
drawLabel($im, 170, $y, 'Totaal opgewerkte energie', $fontArial, 24, $white);
$y+=$cellh;
drawLabel($im, 170, $y, 'CO2 emissie reductie', $fontArial, 24, $white);
$y+=$cellh;
drawLabel($im, 170, $y, 'Kosten besparing', $fontArial, 24, $white);

$total = solarTotal();
$price = getSetting("energy_price");
$peak = solarPeakPower();
$part = preg_split('/-/', firstDay());
$startDate = $part[2].'-'.$part[1].'-'.$part[0];
		
$y=120;
drawLabel($im, 600, $y, $startDate, $fontArial, 24, $white);
$y+=$cellh;
drawLabel($im, 600, $y, round(($peak/1000),2).' kW', $fontArial, 24, $white);
$y+=$cellh;
drawLabel($im, 600, $y, date("d-m-Y"), $fontArial, 24, $white);
$y+=$cellh;
drawLabel($im, 600, $y, round(($total/1000),2).' MWh', $fontArial, 24, $white);
$y+=$cellh;
drawLabel($im, 600, $y, round(($total*$kwh_to_co2_factor)/1000,2).' ton', $fontArial, 24, $white);
$y+=$cellh;
